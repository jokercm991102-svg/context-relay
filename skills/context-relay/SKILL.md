...
5. Show Confirmation complete — Running. Creating the local handoff bundle now. Resolve `scripts/run_context_relay.py` relative to this `SKILL.md`. Use only that resolved wrapper; do not invoke the core CLI directly. Run its `create` command with the project, confirmed objective, and each of the no more than five confirmed next steps. Do not pass `--session` or `--include-text` in the beginner flow.
...
3. Show Confirmation complete — Running. Verifying the project and handoff bundle state now. Resolve `scripts/run_context_relay.py` relative to this `SKILL.md` and run its `resume` command before reading bundle contents.
